 ...
 */
public class DomBotComplexInfo extends Responder {
    @Override
    public boolean response(DomBotResponse response, DomBotResponseThread thread) {
        ...
        if(!response.hasArgStartsWith("about")) return true;
        if(!response.hasArgStartsWith("you")) return true;
        talk(new String[] {
            "My Name's DomBot, I was programmed by Dominic, I am not very smart, but I try, thanks for talking to me :) I get lonely."
        });
        return false;
    }
}
