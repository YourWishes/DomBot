 ...
 */
public class DomBotWhatTime extends Responder {
    @Override
    public boolean response(DomBotResponse response, DomBotResponseThread thread) {
        ...
        if(!response.hasArgStartsWith("time") && !response.hasArgStartsWith("date")) return true;
        talk(new String[] {
            "It's " + Base.getHumanDate(new Date())
        });
        return false;
    }
}
